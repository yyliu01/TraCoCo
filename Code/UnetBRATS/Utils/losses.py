...
def semi_dice_loss(scores, targets,
                   smooth=1e-5, threshold=0.6):
    """
    scores: prob
    targets: logits
    """
    
    targets_prob = torch.softmax(targets, dim=1)
    weight = targets_prob.max(1)[0]
    mask = (weight >= threshold)
    targets = (torch.argmax(targets, dim=1) == 1)
    targets = targets.float()
    intersect = torch.sum(scores[mask] * targets[mask])
    y_sum = torch.sum(targets[mask] * targets[mask])
    z_sum = torch.sum(scores[mask] * scores[mask])
    loss = 1 - (2 * intersect + smooth) / (z_sum + y_sum + smooth)
    return loss.mean()


# peer-based negative learning with confident region
def semi_crc_loss(inputs, targets, threshold=0.6, neg_threshold=0.3, conf_mask=True):
    if not conf_mask:
