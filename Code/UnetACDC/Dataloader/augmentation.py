...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if label.shape[0] < self.output_size[0] / 2 and label.shape[1] < self.output_size[1] / 2:
            ...
            x, y = image.shape
            image = zoom(image, (self.resolution_size[0] / x, self.resolution_size[1] / y), order=0)
            label = zoom(label, (self.resolution_size[0] / x, self.resolution_size[1] / y), order=0)
            x, y = image.shape
            cons_image = image.copy()
            cons_label = label.copy()
            w1 = numpy.random.randint(0, x - self.output_size[0])
            h1 = numpy.random.randint(0, y - self.output_size[1])
            cons_start_x = w1
            cons_start_y = h1
