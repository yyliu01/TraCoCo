...
class Tensorboard:
    ...
    def produce_2d_slice(self, image, label, pred):
        ...
        num = image.shape[0]
        image = image
        grid_image = torchvision.utils.make_grid(image, num, normalize=True)
        wandb.log({"volume_slice": [wandb.Image(grid_image,
                                                caption="x")]})

        outputs = torch.argmax(torch.softmax(pred, 1), dim=1).unsqueeze(1)
        grid_image = torchvision.utils.make_grid(outputs.float(), num, normalize=False)
        wandb.log({"output_slice": [wandb.Image(grid_image,
                                                caption="y_tilde")]})

        grid_image = torchvision.utils.make_grid(label.unsqueeze(1).float(), num, normalize=False)
    ...
    def save_ckpt(self, model, name):
        ...
        print("saving to ... {}".format(os.path.join(self.ckpt_path, name)))
        return
