    ...
    def train_epoch(self, model1, model2, optimizer1, optimizer2, iter_num=0):
        ...
        for batch_idx, (normal_batch, cons_batch) in enumerate(self.train_loader):
            if iter_num >= self.config.max_iterations:
                ...
                iter_num = iter_num + 1
            volume_batch, label_batch = normal_batch['data'], normal_batch['label']
            cons_volume_batch, cons_label_batch = cons_batch['data'], cons_batch['label']
            ...
            sup_outputs1, _ = model1((volume_batch + noise).clamp(min=.0, max=1.))
            noise = torch.zeros_like(volume_batch).uniform_(-.05, .05)
            cons_outputs1, _ = model1((cons_volume_batch + noise).clamp(min=.0, max=1.))
            ...
            for i in range(0, cons_outputs1.shape[0]):
                ...
                spatial_loss1 += F.kl_div(torch.log(torch.softmax(a, dim=1)), torch.softmax(b, dim=1),
                                          reduction='none').mean()
            ...
            """ supervised part """
            # calculate the loss
            sup_ce_loss1 = F.cross_entropy(sup_outputs1[:self.labeled_num],
            ...
            noise = torch.zeros_like(volume_batch).uniform_(-.05, .05)
            unsup_outputs1, _ = model1((volume_batch + noise).clamp(min=.0, max=1.))
            ...
            # spatial-consistent
            noise = torch.zeros_like(volume_batch).uniform_(-.05, .05)
            sup_outputs2, _ = model2((volume_batch + noise).clamp(min=.0, max=1.))
            noise = torch.zeros_like(cons_volume_batch).uniform_(-.05, .05)
            cons_outputs2, _ = model2((cons_volume_batch + noise).clamp(min=.0, max=1.))
            ...
            for i in range(0, cons_outputs2.shape[0]):
                ...
                spatial_loss2 += F.kl_div(torch.log(torch.softmax(a, dim=1)), torch.softmax(b, dim=1),
                                          reduction='none').mean()
            ...
            """ supervised part """
            # calculate the loss
            sup_ce_loss2 = F.cross_entropy(sup_outputs2[:self.labeled_num],
                                           label_batch[:self.labeled_num]).mean()
            ...
            noise = torch.zeros_like(volume_batch).uniform_(-.05, .05)
            unsup_outputs2, _ = model2((cut_mix_input2 + noise).clamp(min=.0, max=1.))
            ...
            self.logger.info('model1 iteration %d : loss : %f unsup_loss: %f, loss_ce: %f loss_dice: %f spatial: %f' %
                             (iter_num, loss1.item(), loss1_crc.item(), sup_ce_loss1.item(),
            ...
            self.logger.info('model2 iteration %d : loss : %f unsup_loss: %f, loss_ce: %f loss_dice: %f spatial: %f' %
                             (iter_num, loss2.item(), loss2_crc.item(), sup_ce_loss2.item(),
    ...
    def validate(self, epoch, model, model_id):
        ...
        for batch_idx in tbar:
            ...
            y_tilde, y_hat = test_single_case(model, x,
                                              ...
                                              patch_size=(96, 96, 96), num_classes=self.config.num_classes)
            if np.sum(y_tilde) == 0:
