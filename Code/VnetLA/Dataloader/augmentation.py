...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        # no-overlap issues
        cons_start_x = cons_start_x + int(w1/2) if w1 - cons_start_x > 112 else cons_start_x
        cons_start_y = cons_start_y + int(h1/2) if h1 - cons_start_y > 112 else cons_start_y
        ...
        a = image[0 if cons_start_x < w1 else cons_start_x-w1:112-(w1-cons_start_x) if cons_start_x < w1 else 112,
            0 if cons_start_y < h1 else cons_start_y-h1:112-(h1-cons_start_y) if cons_start_y < h1 else 112,
            0 if cons_start_z < d1 else cons_start_z-d1:80-(d1-cons_start_z) if cons_start_z < d1 else 80]

        b = cons_image[0 if cons_start_x > w1 else w1-cons_start_x:112-(cons_start_x-w1) if cons_start_x > w1 else 112,
            0 if cons_start_y > h1 else h1-cons_start_y:112-(cons_start_y-h1) if cons_start_y > h1 else 112,
            0 if cons_start_z > d1 else d1-cons_start_z:80-(cons_start_z-d1) if cons_start_z > d1 else 80]

        assert numpy.all(numpy.equal(a, b)), "?"
        return {'image': image, 'label': label, 'cons_image': cons_image, 'cons_label': cons_label,
                'normal_range_x': [0 if cons_start_x < w1 else cons_start_x-w1, 112-(w1-cons_start_x) if cons_start_x < w1 else 112],
                'cons_range_x': [0 if cons_start_x > w1 else w1-cons_start_x, 112-(cons_start_x-w1) if cons_start_x > w1 else 112],
                'normal_range_y': [ 0 if cons_start_y < h1 else cons_start_y-h1, 112-(h1-cons_start_y) if cons_start_y < h1 else 112],
                'cons_range_y': [0 if cons_start_y > h1 else h1-cons_start_y, 112-(cons_start_y-h1) if cons_start_y > h1 else 112,],
                'normal_range_z': [0 if cons_start_z < d1 else cons_start_z-d1, 80-(d1-cons_start_z) if cons_start_z < d1 else 80],
                'cons_range_z': [0 if cons_start_z > d1 else d1-cons_start_z, 80-(cons_start_z-d1) if cons_start_z > d1 else 80]}
