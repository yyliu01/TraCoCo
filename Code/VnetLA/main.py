...
def main(args):
    ...
    #if args.pvc:
    #    download_la_unzip(data_dir="Left_Atrium", prefix="yy/exercise_3/")

    train_set = LAHeartDataset(os.path.join(config.code_path, "Dataloader"),
                               config.data_path,
                               split="train", config=config)

    # merge both labelled & unlabelled sampler to same batch
    batch_sampler = TwoStreamBatchSampler(list(range(config.labeled_num)), list(range(config.labeled_num, 80)), config.batch_size,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--labeled_num", default=16, type=int,
                        help="number of the labelled sample")
    parser.add_argument("--max_iterations", default=9000, type=int,
